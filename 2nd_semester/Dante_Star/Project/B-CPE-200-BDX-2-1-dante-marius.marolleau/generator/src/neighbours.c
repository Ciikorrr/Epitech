...
void neighbours_lenght_positive(prims *maze, border_t *neighbours)
{
    ...
    if (neighbours->lenght > 0) {
        ...
        maze->base.maze[neighbours->array[random_neighbour].coords.x]
        [neighbours->array[random_neighbour].coords.y].visited = true;
        maze->base.maze[neighbours->array[random_neighbour].coords.x]
        [neighbours->array[random_neighbour].coords.y].cell_case = '*';
    }
}
...
void random_neighbours(prims *maze, int idx)
{
    ...
    add_neighbours(maze, &neighbours, (coords_t){tmp.x + 1, tmp.y},
    (coords_t){tmp.x + 2, tmp.y});
    add_neighbours(maze, &neighbours, (coords_t){tmp.x - 1, tmp.y},
    (coords_t){tmp.x - 2, tmp.y});
    add_neighbours(maze, &neighbours, (coords_t){tmp.x, tmp.y + 1},
    (coords_t){tmp.x, tmp.y + 2});
    add_neighbours(maze, &neighbours, (coords_t){tmp.x, tmp.y - 1},
    (coords_t){tmp.x, tmp.y - 2});
