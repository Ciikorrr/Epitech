...
int get_precision(attributes_t flag_att)
{
...
int flag_f(va_list list, int count, attributes_t flag_att)
{
    int i = 0, j = 0, precision = get_precision(flag_att);
    intmax_t x = va_arg(list, double) * (my_compute_power_rec(10, precision));
    if (x < 0)
        x = x * -1;
    char *str = malloc(sizeof(char) * (my_numberlen(x) + 1));
    memset(str, '\0', sizeof(char) * (my_numberlen(x) + 1));
    while (x) {
        str[i++] = (x % 10) + 48;
        x = x / 10;
    }
