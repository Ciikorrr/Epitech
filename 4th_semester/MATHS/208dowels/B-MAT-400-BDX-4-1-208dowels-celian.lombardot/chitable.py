...
def chi2table():
    table = [
        ["nu","99%","90%","80%","70%","60%","50%","40%","30%","20%","10%","5%","2%","1%"],
...
def getinterval(chi, deg):
    table = chi2table()
    percent = table[0]
    line = table[deg]

    i = 0
    while (i + 1 < len(line) and chi - line[i + 1] > 0):
        i += 1
    if (i + 1 == len(line)):
        return percent[i], 0
    if i == 0:
        return 0, percent[i + 1]
    return percent[i], percent[i+1]


if __name__ == "__main__":
    chi = 2.029
    mini = getinterval(chi, 5)
    print(mini)
